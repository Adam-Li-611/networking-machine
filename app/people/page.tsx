...
export default async function PeoplePage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  return (
    <div>
      <PageHeader
        title="People"
        description="Relationship memory, contact quality, and recruiting context."
        actions={
          <>
            ...
            <ButtonLink href="/contacts/new" size="sm"><Plus className="h-4 w-4" /> Add person</ButtonLink>
          </>
        }
      />
      <form className="mb-3 grid gap-2 rounded-lg border bg-card p-3 md:grid-cols-6">
        <Input name="search" placeholder="Search people..." defaultValue={typeof params.search === "string" ? params.search : ""} />
